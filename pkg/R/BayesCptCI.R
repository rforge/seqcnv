BayesCptCI <-
function(cases, controls, CBSRes, stepSize=5, alpha=0.05, epsilon=10^-4, epsCDF=10^-4, verbose=FALSE) {
	...
	if(length(tauHat) < 3) {
		...
		for(j in 2:(nTauHatReal+1)) {
			...
			while(k > tauBoundL && wkCur > epsilon) {
				...
				if(verbose) {
					print(c(k, sTauLCur, tTauLCur-sTauLCur, sTauRCur, tTauRCur-sTauRCur, wkCur))
				}
				if(wkCur > epsilon) {
		...
		timeCIMix = proc.time()[3]  - timeCIMix
